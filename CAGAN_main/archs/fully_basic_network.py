...
class DisCell(nn.Module):
    def __init__(self, args, in_channels, out_channels, hidden_channels=None, activation=nn.ReLU(), genotype=None):
        super(DisCell, self).__init__()
        if genotype[0] > 0:
            self.c0 = MixedOp(out_channels, out_channels, 1, True, True, [PRIMITIVES[genotype[0]]])
        if genotype[1] > 0:
            self.c1 = MixedOp(out_channels, out_channels, 1, True, True, [PRIMITIVES[genotype[1]]])
        if genotype[2] > 0:
            self.c2 = MixedOp(out_channels, out_channels, 1, True, True, [PRIMITIVES[genotype[2]]])
        if genotype[3] > 0:
            self.c3 = MixedOp(out_channels, out_channels, 1, True, True, [PRIMITIVES[genotype[3]]])
        if genotype[4] > 0:
            self.c4 = MixedOp(out_channels, out_channels, 1, True, False, [PRIMITIVES[genotype[4]]])
        if genotype[5]>=0:
            self.down0 = MixedDown(in_channels, out_channels, 2, True, True, [PRIMITIVES_down[genotype[5]]])
            self.down1 = MixedDown(in_channels, out_channels, 2, True, True, [PRIMITIVES_down[genotype[6]]])
